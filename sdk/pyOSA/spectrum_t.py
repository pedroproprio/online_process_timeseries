...
class spectrum_t(c.Structure):
    ...
    def __init__(self,
        ...
        """ Allocates an empty spectrum_t with the lengths specified.

        Parameters:
            allocated_length_I:
                The length to allocate for the I field.

            allocated_length_x (default: 0):
                The length to allocate for the x field.

            allocated_length_phi (default: 0):
                The length to allocate for the phi field.

            allocated_length_log (default: 0):
                The length to allocate for the logData field.

        For an interferogram it is sufficent to allocate the I field.
        For a spectrum, both the I and the x fields need to be allocated.
        """
        self.allocatedLengthI = allocated_length_I
        array_I = (c.c_float * allocated_length_I)()
        self.I = c.cast(array_I, c.POINTER(c.c_float))

        self.allocatedLengthx = allocated_length_x
        array_x = (c.c_float * allocated_length_x)()
        self.x = c.cast(array_x, c.POINTER(c.c_float))

        self.allocatedLengthPhi = allocated_length_phi
        array_phi = (c.c_float * allocated_length_phi)()
        self.phi = c.cast(array_phi, c.POINTER(c.c_float))

        self.allocatedLengthLog = allocated_length_log
        array_logData = (c.c_byte * allocated_length_log)()
        self.logData = c.cast(array_logData, c.c_void_p)

        self.hdrversion = constants.spectrum_t_hdr_version
        self.hdrversionOriginal = constants.spectrum_t_hdr_version
        self.hdrsize = c.sizeof(spectrum_t)
    ...
    def _is_interferogram_clipped(self) -> bool:
        ...
        if self.is_interferogram():
            ...
            if not self.isStitched:
                y_min_list = [self.minInterferogram]
    ...
    def get_datetime(self) -> datetime.datetime:
        ...
        .. code-block:: python
            ...
            datestr = spectrum.get_datetime().isoformat()
        """
        hour = self.time // 1000000
        minute = (self.time % 1000000) // 10000
        second = (self.time % 10000) // 100
        millisecond = (self.time % 100) * 10
    ...
    def calculate_spectrum(self,
                           apodization_type='Hann', #constants.data_defines['APODIZATION_HANN'],
                           zero_fill_factor=0
                           ) -> "spectrum_t":
        """ Returns a spectrum corresponding to the given interferogram and calculation options.

        Parameters:
            apodization_type (Default: 'Hann'):
                The interferogram apodization used in the spectrum calculation.
                Valid apodizations are listed in the acquire method in the manual.

            zero_fill_factor (Default: 0):
                The zerofill factor used in the spectrum calculation.
                Must equal 0, 1, or 2.
        """
        # Smoothing can be used as a post-process of the spectrum calculation.
        #    smoothing_type (Default: SMOOTHING_NONE):
        #        The smoothing type applied on the calculated spectrum.
        #        Valid values are given by constants.data_defines[SMOOTHING_...]
        #
        #    smoothing_parameter_1 (Default: 0) & smoothing_parameter_2 (Default: 0):
        #        Parameters used by the smoothing function.
        #        These values only matters if smoothing_type differs from SMOOTHING_NONE.
        smoothing_type=constants.data_defines['SMOOTHING_NONE']
        smoothing_parameter_1=0
        smoothing_parameter_2=0

        if not self.is_interferogram():
            error_message = "Invalid input: calculate_spectrum() can only calculate a spectrum corresponding to an interferogram"
            logger.error(error_message)
            raise ValueError(error_message)
        
        apodization_type_nr = constants.convert_apodization_string_to_number(apodization_type)
 
        # Allocate memory for the spectrum; this is the maximum possible length
        desired_spectrum_length = math.ceil((2 ** zero_fill_factor) * self.length / 2) + 1
        spectrum = spectrum_t(allocated_length_I=desired_spectrum_length, allocated_length_x=desired_spectrum_length)
        
        FTSLib.FTS_CalculateSpectrumFromInterferogram.argtypes = (c.POINTER(spectrum_t), c.POINTER(spectrum_t), c.c_ubyte, c.c_ubyte, c.c_ushort, c.c_ushort, c.c_ushort)
        status = FTSLib.FTS_CalculateSpectrumFromInterferogram(self, spectrum, apodization_type_nr, zero_fill_factor, smoothing_type, smoothing_parameter_1, smoothing_parameter_2)

        if status != 0:
            error_message = f"Error in FTS_CalculateSpectrumFromInterferogram, Status: {constants.err_msg(status)}"
            logger.error(error_message)
            raise Exception(error_message)
        
        # Avoid too much allocated memory (create a spectrum copy which allocated lengths agree with the length of the spectrum)
        allocated_length_x   = spectrum.length if (spectrum.allocatedLengthx   > 0) else 0
        allocated_length_phi = spectrum.length if (spectrum.allocatedLengthPhi > 0) else 0
        spectrum_minimal_allocation = spectrum_t(allocated_length_I=spectrum.length, allocated_length_x=allocated_length_x, allocated_length_phi=allocated_length_phi, allocated_length_log=spectrum.allocatedLengthLog)
        FTSLib.FTS_CopySpectrum.argtypes = (c.POINTER(spectrum_t), c.POINTER(spectrum_t))
        FTSLib.FTS_CopySpectrum(spectrum_minimal_allocation, spectrum)

        return spectrum_minimal_allocation
