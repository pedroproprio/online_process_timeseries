...
class core(object):
    ...
    def load_spf2_file(filename: str) -> typing.List[spectrum_t]:
        ...
        spec_list = core._read_spectra(filename_utf8, number_of_acquisitions)
        return spec_list
    ...
    @staticmethod
    def _read_spectra(filename_utf8: bytes, number_of_acquisitions: int):
        """
        Helper method when loading spectrum_t from files, reads, loads, copies, spectra
        """
        FILEFORMAT = 2  # SPF2
        core._allocate_phi_x_arrays(number_of_acquisitions)
        FTSLib.FTS_ReadSpectra_Traces.argtypes = (
            c.c_uint,
            c.c_char_p,
            c.c_int,
            c.POINTER(c.c_uint)
        ...
        num_spectra_read = c.c_uint()
        status = FTSLib.FTS_ReadSpectra_Traces(number_of_acquisitions, c.c_char_p(filename_utf8), FILEFORMAT, c.byref(num_spectra_read))
        if status != 0:
            raise Exception("Error reading spectrum from file")
        
        array_traces = core._get_array_traces()
        spec_list = []
        for index_to_read in range(num_spectra_read.value):
            spec = spectrum_t(
                ...
                allocated_length_phi=array_traces[index_to_read].contents.allocatedLengthPhi,
                allocated_length_log=array_traces[index_to_read].contents.allocatedLengthLog
            )
    ...
    def save_spf2_file(
        ...
        if isinstance(specs, spectrum_t):
            core._write_spectrum_to_file(specs, filename, FILEFORMAT)
        elif isinstance(specs, list):
            core._write_spectra_to_file(specs, filename, FILEFORMAT)
        elif isinstance(specs, dict):
            speclist = list(specs.values())
            core._write_spectra_to_file(speclist, filename, FILEFORMAT)
        else:
            raise TypeError("Invalid parameter in save spf2")

    @staticmethod
    def _write_spectrum_to_file(spec: spectrum_t, filename: str, FILEFORMAT: int):
        """
    ...
    def _write_spectra_to_file(
        ...
        for i, spec in enumerate(specs):
            if not isinstance(spec, spectrum_t):
                raise TypeError("Invalid input to _write_spectra_to_file, list in not of spectrum_t")
    ...
    @staticmethod
    def write_tracelog(idnr: typing.Union[int, None] = None, path: typing.Union[str, None] = None) -> None:
        """
        Creates a trace log file with debug information.

        The log is saved as `Tracelog_<idnr>.txt`, where `idnr` is either the provided
        integer or a timestamp-based ID (YYMMDDHHMM).

        Parameters:
        -----------
        idnr : int, optional
            Unique identifier for the log file. Defaults to a timestamp if not provided.
        path : str, optional
            Directory to save the log file. Defaults to the current working directoy
        """
        if idnr is None:
            idnr = int(datetime.now().strftime('%y%m%d%H%M'))
        if path is None:
            path = os.getcwd()
        if not isinstance(idnr, int):
            raise TypeError("idnr needs to be an integer")
        if not isinstance(path, str):
            raise TypeError("Tracelog path needs to be a string")
        FTSLib.FTS_SetOperatorLibrary(c.c_char_p(path.encode()))
        FTSLib.FTS_StartWriteTraceLog(idnr)
